...
// for L.size > 1, v is also essentially a scratch vector
	...
	if(printnewton){ 
		...
		for(ih=0; ih<Nm; ih++){
			...
			double c = get_c( ms[ih] );
			if( c < 0.0 ){
				SetLast2( ms[ih]->vpsi, creal(w ), -c );
				// prevents negative c's for convenience (issue with printed mode files with negative c's being treated as a below threshold mode for the next run
			}


			PetscPrintf(PETSC_COMM_WORLD, "%s at D = %g: w = %1.15g", ms[ih]->name,  geo->D, creal(w));
			if(! ms[ih]->lasing)
